resource "aws_ecr_repository" "todo_app" {
  name = "todo-app" # Naming my repository
}

resource "aws_ecs_cluster" "todo_app_cluster" {
  name = "todo-app-cluster" # Naming the cluster
}

resource "aws_ecs_task_definition" "todo_app_task" {
  family                   = "todo-app-task" # Naming our first task
...
resource "aws_ecs_service" "todo_app_service" {
  name            = "todo-app-services"                       # Naming our first service
  cluster         = aws_ecs_cluster.todo_app_cluster.id       # Referencing our created Cluster
  task_definition = aws_ecs_task_definition.todo_app_task.arn # Referencing the task our service will spin up
  launch_type     = "FARGATE"
  ...
  load_balancer {
    ...
    container_name   = aws_ecs_task_definition.todo_app_task.family
